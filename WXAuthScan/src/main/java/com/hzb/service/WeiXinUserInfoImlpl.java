 ...
 **/
@Service
public class WeiXinUserInfoImlpl implements WeiXinUserInfoService {
	/**
	 * 获取微信用户的信息
	 * 
	 * @param accessToken
	...
	public WeiXinUser getUserInfo(String accessToken, String openId) {
		...
		// 拼接获取用户信息接口的请求地址
		String requestUrl = ProjectConst.GET_WEIXIN_USER_URL.replace("ACCESS_TOKEN", accessToken).replace("OPENID",
				openId);
		// 获取用户信息(返回的是Json格式内容)
	...
	public Map<String, String> oauth2GetOpenid(String code) {
		...
		// 拼接用户授权接口信息
		String requestUrl = ProjectConst.SNS_OAUTH2.replace("APPID", appid).replace("SECRET", appsecret)
				.replace("CODE", code);
		// 存储获取到的授权字段信息
		Map<String, String> result = new HashMap<String, String>();
		try {
			JSONObject OpenidJSONO = HttpUtil.doGetstr(requestUrl);
